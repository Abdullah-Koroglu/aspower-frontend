        ...
        <div className="flex flex-col h-fit items-start mx-auto max-w-[70rem]">
          ...
          <div className="mt-10 mb-20 flex flex-col">
            {pageData.data.filter(item => item.attributes.tip === 'sertifika')?.map((item, index) => (
              <Link target="_blank" rel="noopener noreferrer" href={`${process.env.NEXT_PUBLIC_IMAGE_URL}${item.attributes.Dokuman.data}`} key={index} className="text-xl font-base mb-4 text-[#005770]">{item.attributes.Sertifika}</Link>
            ))}
          </div>
